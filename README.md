...
2. **`Run_Attractor_Analysis.m`** - the main pipeline. Loads `cfg`, then
   ...
   `cfg.RESULTS_DIR`.
2b. **`Run_Evoked_Analysis.m`** - companion pipeline for a single-stimulus recording (Baseline + Evoked only, no Recovery epoch).
...
3. **`Run_Cohort_Analysis.m`** - run this AFTER `Run_Attractor_Analysis.m` or `Run_Evoked_Analysis.m`
